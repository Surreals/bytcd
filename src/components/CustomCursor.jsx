...
// Define the ArrowPointer class to encapsulate cursor logic
class ArrowPointer {
  constructor(cursorElement) {
    ...
    this.cursorStyle = {
      ...
      userSelect: 'none',
      pointerEvents: 'none',
      opacity: 0, // Start hidden, will be set to 1 in init
    };
  ...
  init(el, style) {
    ...
    // Use a slight delay to ensure CSS transition applies for fade-in
    setTimeout(() => {
...
const CustomCursor = () => {
  ...
  useEffect(() => {
    if (cursorRef.current) {
      cursorInstanceRef.current = new ArrowPointer(cursorRef.current);

      const handleMouseMove = (event) => {
        if (cursorInstanceRef.current) {
          cursorInstanceRef.current.move(event);
        }
      };

      const handleTouchMove = (event) => {
        if (cursorInstanceRef.current && event.touches.length > 0) {
          cursorInstanceRef.current.move(event.touches[0]);
        }
      };

      const handleClick = () => {
        if (cursorInstanceRef.current && typeof cursorInstanceRef.current.click === 'function') {
          cursorInstanceRef.current.click();
        }
      };
      ...
      return () => {
        ...
        if (cursorInstanceRef.current) {
          cursorInstanceRef.current.hidden(); // Ensure cursor fades out on unmount
        }
      };
    }
  }, []);
